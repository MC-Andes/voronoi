...
def recortar_poliedro_contra_caja(poliedro_caras, ancho, alto, profundo):
    poliedro_recortado = []
    for cara in poliedro_caras:
        cara_nueva = []
        for x, y, z in cara:
            cx = max(0.0, min(ancho, x))
            cy = max(0.0, min(alto, y))
            cz = max(0.0, min(profundo, z))
            cara_nueva.append((cx, cy, cz))
        
        cara_limpia = []
        for v in cara_nueva:
            if not cara_limpia or v != cara_limpia[-1]:
                cara_limpia.append(v)
                
        if len(cara_limpia) >= 3:
            poliedro_recortado.append(cara_limpia)
    return poliedro_recortado
...
def voronoi_3d(rango_caras=(4, 25), ancho=100, alto=100, profundo=100, num_particulas=80):
    cota_inf, cota_sup = rango_caras
    points = generar_espacio_perturbado(num_particulas, ancho, alto, profundo)
    tetraedros, points = delaunay_3d_con_params(points)
    poliedros = []
    ...
    for p in points:
        ...
        for tetra in tetraedros:
            if any(v in supa_tetra for v in tetra):
                continue
            if p in tetra:
                center, _ = esfera_circunscrita(tetra)
                if center is not None:
                    if not (-50 <= center[0] <= ancho + 50 and 
                            -50 <= center[1] <= alto + 50 and 
                            -50 <= center[2] <= profundo + 50):
                        continue
        ...
        if cota_inf <= len(caras_poliedro) <= cota_sup:
            caras_recortadas = recortar_poliedro_contra_caja(caras_poliedro, ancho, alto, profundo)
            poliedro_separado = contraer_3d(caras_recortadas, p)
...
def visualizar_voronoi_solido(poliedros, puntos_semilla):
    ...
    # Listas para guardar las aristas de una sola vez
    edge_x, edge_y, edge_z = [], [], []

    # 1. Dibujar las caras de los poliedros (Estilo Sólido Mate)
    for poliedro in poliedros:
        # Colores pastel/sólidos aleatorios pero controlados (evitando colores muy oscuros)
        ...
        for cara in poliedro:
            ...
            for i, v in enumerate(cara):
                ...
                # Extraemos las aristas para el wireframe oscuro
            ...
            # Triangulación interna para Plotly
        ...
        if x_all:
            fig.add_trace(go.Mesh3d(
                ...
                color=color,
                opacity=0.95, # Opacidad casi total para un look sólido
                flatshading=True,
                # Ajustes de iluminación para un efecto mate "tipo plastilina"
                lighting=dict(ambient=0.6, diffuse=0.8, roughness=0.8, specular=0.1),
                hoverinfo='none'
            ))

    # 2. Dibujar las aristas (Líneas negras/oscuras bien definidas)
    fig.add_trace(go.Scatter3d(
        x=edge_x, y=edge_y, z=edge_z,
        mode='lines',
        line=dict(color='rgb(30, 30, 30)', width=4), # Grosor y color oscuro
        hoverinfo='none',
        showlegend=False
    ))

    # 3. Puntos semilla (opcionales, más discretos)
    ...
    # 4. Estética de la escena: Fondo claro y limpio
    fig.update_layout(
...
if __name__ == "__main__":
    ...
    print(f"Generando {NUM_PARTICULAS} celdas...")
