...
def delaunay_triangulation():
    ...
    # --- FILTRADO DE TRIÁNGULOS DEL SUPER-TRIÁNGULO ---
    triangulos_limpios = []
...
poliedros, points = voronoi((3, 9))

# Configuramos la ventana de gráficos
fig, ax = plt.subplots(figsize=(8, 8))

# 1. Dibujar los polígonos de Voronoi (las celdas)
for poligono in poliedros:
    if len(poligono) > 0:
        # Añadimos el primer punto al final para cerrar el polígono visualmente
        poly_cerrado = poligono + [poligono[0]]
        xs = [pt[0] for pt in poly_cerrado]
        ys = [pt[1] for pt in poly_cerrado]
        ax.plot(xs, ys, color='blue', alpha=0.6, linewidth=1.5)
        ax.fill(xs, ys, color='lightblue', alpha=0.2) # Relleno opcional para ver las celdas

# 2. Dibujar las semillas (puntos originales)
pxs = [p[0] for p in points]
pys = [p[1] for p in points]
ax.scatter(pxs, pys, color='red', zorder=5, s=20, label='Semillas')

# Ajustes estéticos del gráfico
plt.xlim(0, 100)
plt.ylim(0, 100)
ax.set_aspect('equal')
plt.title("Diagrama de Voronoi generado")
plt.xlabel("X")
plt.ylabel("Y")
plt.legend()
